#!/bin/bash
# Launch TRELLIS.2 Sparse Structure Flow fine-tuning on RunPod
#
# Prerequisites:
...
#
# Usage: bash train_trellis_ss_flow.sh [--resume STEP]

set -euo pipefail

WORKSPACE="${WORKSPACE_DIR:-$HOME/workspace}"
TRELLIS_DIR="$WORKSPACE/repos/TRELLIS.2"
CONFIG="$WORKSPACE/repos/3DAI/configs/trellis/ss_flow_finetune.json"
OUTPUT_DIR="$WORKSPACE/checkpoints/trellis/finetuned/ss_flow"
DATA_DIR='{"Forge3D_Custom": {"base": "'$WORKSPACE'/data/trellis", "ss_latent": "ss_latent", "render_cond": "render_cond", "asset_stats": "asset_stats"}}'
NUM_GPUS="${NUM_GPUS:-1}"
RESUME_STEP="${2:-}"

echo "========================================="
echo "TRELLIS.2 Sparse Structure Flow Fine-tuning"
echo "========================================="
echo "Config:     $CONFIG"
...
[ -d "$WORKSPACE/data/trellis/ss_latent" ] || { echo "ERROR: Training data not found. Run prep_trellis_data.sh first."; exit 1; }
